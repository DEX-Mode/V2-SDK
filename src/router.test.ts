...
import { CurrencyAmount, Percent, ETHER, Token, WETH9 } from 'udonswap-core'
...
describe('Router', () => {
  ...
  const pair_weth_0 = new Pair(
    CurrencyAmount.fromRawAmount(WETH9[919], '1000'),
    ...
    describe('exact in', () => {
      it.only('ether to token1', () => {
        const result = Router.swapCallParameters(
          Trade.exactIn(
            new Route([pair_weth_0, pair_0_1], ETHERIUM, token1),
        ...
        expect(result.args.slice(0, -1)).toEqual([
          '0x51',
          [WETH9[919].address, token0.address, token1.address],
      ...
      it('token1 to ether', () => {
        const result = Router.swapCallParameters(
          Trade.exactIn(
            new Route([pair_0_1, pair_weth_0], token1, ETHERIUM),
        ...
        expect(result.args.slice(0, -1)).toEqual([
          '0x64',
          '0x51',
          [token1.address, token0.address, WETH9[919].address],
          '0x0000000000000000000000000000000000000004'
        ])
